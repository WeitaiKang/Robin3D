...
def create_dataset(config):
    if config.evaluate:
        ...
        train_files = []
        train_dataset_names = []
        print(f'----- total train set: {len(config.train_tag.split("#"))} ---')
        for train_name in config.train_tag.split('#'):
            if train_name not in config.train_file_dict:
                print(f'----- {train_name} not in train_file_dict ---')
        ...
        for train_file, train_name in zip(train_files, train_dataset_names):
            datasets.append(TrainDataset(ann_list=train_file, dataset_name=train_name, config=config))
            print(f'----- {train_name} train set: {len(datasets[-1])} ---')
        dataset = ConcatDataset(datasets)
        print(f'----- total train set: {len(dataset)} ---')
