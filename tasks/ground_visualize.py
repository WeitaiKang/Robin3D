...
# data
SCANNET_ROOT = "/data/kangweitai/3D/scannet/scans/"
...
if __name__ == "__main__":
    attrs = torch.load("/data/kangweitai/3D/chat3d-anno/scannet/scannet_train_attributes.pt")
    scene_id = "scene0623_00" # "scene0407_00" # "scene0126_00" # "scene0520_00" # "scene0489_00" # "scene0040_00"
    scene_bbox = np.array(attrs[scene_id]["locs"])
    scene_object = np.array(attrs[scene_id]["objects"])

    output_scene_dir = f"visualize/{scene_id}"
    ...
    for obj_id in range(len(scene_bbox)):
        ...
        output_scene_dir = f"visualize/{scene_id}/{scene_object[obj_id]}"
        os.makedirs(output_scene_dir, exist_ok=True)
