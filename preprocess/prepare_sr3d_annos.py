...
for split in ["val"]: # ,  "train"
    ...
    print(f'--- original {split} json has {len(annos)} annotations ---')
    new_annos = []
    ...
    if split == 'train':
        print(f'--- new sr3d+_{segmentor}_{split}{version}.json has {len(new_annos)} annotations ---')
        ...
        save_path = os.path.join(args.save_dir, f"sr3d+_{segmentor}_{split}{version}.json")
        with open(save_path, "w") as f:
            json.dump(new_annos, f, indent=4)
    else:
        print(f'--- new sr3d_{segmentor}_{split}{version}.json has {len(new_annos)} annotations ---')
        print(f"percent iou@0.25 of new_annos: {iou25_count / len(new_annos)}")
        print(f"percent iou@0.5 of new_annos: {iou50_count / len(new_annos)}")
        save_path = os.path.join(args.save_dir, f"sr3d_{segmentor}_{split}{version}.json")
        with open(save_path, "w") as f:
            json.dump(new_annos, f, indent=4)

# --- original sr3d+_train.json has 90450 annotations ---
# --- new sr3d+_mask3d_train.json.json has 87358 annotations ---
# percent iou@0.25 of new_annos: 1.0
# percent iou@0.5 of new_annos: 0.9559284782160764

# --- original sr3d+_val.json has 24082 annotations ---
# --- new sr3d+_mask3d_val.json.json has 24082 annotations ---
# percent iou@0.25 of new_annos: 0.8980981646042687
# percent iou@0.5 of new_annos: 0.8070758242670875