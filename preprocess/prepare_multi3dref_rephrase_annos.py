...
for split in ['train']:
    annos = json.load(open(f"/data/kangweitai/3D/annotation/gpt_rephrase/multi3dref_train_rephrase_merged.json"))
    new_annos = []
    ...
    print(f'--- new multi3dref_{segmentor}_{split}{version}_rephrase.json has {len(new_annos)} annotations ---')
    save_path = os.path.join(args.save_dir, f"multi3dref_{segmentor}_{split}{version}_rephrase.json")
