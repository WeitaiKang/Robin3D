...
for split in ["train", "val"]:
    ...
    # print(len(new_annos))
    print(f'--- new scanrefer_{segmentor}_{split}{version}.json has {len(new_annos)} annotations ---')
    print(f"percent iou@0.25 of new_annos: {iou25_count / len(annos)}")
    print(f"percent iou@0.5 of new_annos: {iou50_count / len(annos)}")

    save_path = os.path.join(args.save_dir, f"scanrefer_{segmentor}_{split}{version}.json")
