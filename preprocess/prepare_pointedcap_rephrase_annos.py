...
for split in ["train"]:
    ...
    cap_annos = json.load(open(f"/data/kangweitai/3D/annotation/gpt_rephrase/scanrefer_train_rephrase_merged.json", "r"))
    cap_scene_ids = set()
    ...
    print(f'--- save data num: {len(new_annos)} to pointedcap_{segmentor}_{split}{version}_rephrase.json --- total positive num: {total_positive_num}, total negative click num: {total_negative_click_num} -- total negative category num: {total_negative_category_num} --')
    save_path = os.path.join(args.save_dir, f"pointedcap_{segmentor}_{split}{version}_rephrase.json")
